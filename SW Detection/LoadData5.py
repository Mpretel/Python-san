...
# PARA WAVELET
seconds = 40#50#40#40#40#10 # En este caso deberí haber 2 ondas lentas
...
Y_norm = Y_norm.argmax(axis=1)
#%%
plt.plot(X[offset*200:(seconds+offset)*200])
# plt.plot(X)
...
for k in range(offset,seconds+offset+1):
    ...
    if Y_norm[k-offset+1]==1:
        # Ventana de 250ms
        ejeX = np.linspace((k-offset)*200+Dw,((k-offset)+1)*200+Dw-1,200)
        plt.plot(ejeX, X[k*200+Dw:(k+1)*200+Dw], 'r')

    if Y_norm[k-offset+2]==1:
        # Ventana de 250ms
        ejeX = np.linspace((k-offset)*200+2*Dw,((k-offset)+1)*200+2*Dw-1,200)
        plt.plot(ejeX, X[k*200+2*Dw:(k+1)*200+2*Dw], 'r')

    if Y_norm[k-offset+3]==1:
        # Ventana de 250ms
        ejeX = np.linspace((k-offset)*200+3*Dw,((k-offset)+1)*200+3*Dw-1,200)
